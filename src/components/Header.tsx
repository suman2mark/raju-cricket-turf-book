  ...
  return <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-white shadow-md py-2' : 'bg-transparent py-4'}`}>
      <div className="container mx-auto flex justify-between items-center px-4">
        ...
        <nav className="hidden md:flex items-center space-x-6">
          {['home', 'booking', 'pricing', 'contact', 'location'].map(section => <button key={section} onClick={() => scrollToSection(section)} className={`${isScrolled ? 'text-gray-700 hover:text-primary' : 'text-white/80 hover:text-white'} font-medium relative group transition-colors duration-300`}>
              {translate(section)}
        ...
        <div className="flex items-center space-x-3">
          <Button variant={isScrolled ? "outline" : "secondary"} size="sm" onClick={toggleLanguage} className={`text-xs md:text-sm ${!isScrolled && 'border-white/30 text-white bg-white/10 hover:bg-white/20'} transition-colors duration-300`}>
          ...
          window.open(`https://wa.me/${ADMIN_WHATSAPP_NUMBER}`, '_blank');
            ...
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="mr-2 h-4 w-4">
          ...
          <div className="md:hidden">
            <Button variant={isScrolled ? "ghost" : "secondary"} size="sm" className={`p-1 ${!isScrolled && 'text-white bg-white/10 hover:bg-white/20'}`} onClick={() => {
